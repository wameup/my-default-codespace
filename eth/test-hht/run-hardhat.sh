...
npx hardhat run scripts/deploy.js --network [localhost|goerli|...] # 如果部署到 hardhat network, 省略 --network localhost 也显示成功，即使没有事先启动 node；但在 node 记录里似乎没有上链。localhost以外的网络需要在 hardhat.config.js 中定义。
